...
cd /home/z/my-project/repos/Zenic-Flijo
export WFD_SESSION_SECRET=$(python3 -c "import secrets; print(secrets.token_hex(32))")
export WFD_LICENSE_SECRET=$(python3 -c "import secrets; print(secrets.token_hex(32))")
export WFD_WEB_HOST=0.0.0.0 WFD_WEB_PORT=8080 WFD_WEBHOOK_PORT=8081
export PYTHONPATH=/home/z/my-project/repos/Zenic-Flijo

# Arrancar backend
/home/z/.venv/bin/python3 -m src.main > /tmp/backend.log 2>&1 &
BACKEND_PID=$!
...
curl -s -c /tmp/cookies.txt -X POST http://127.0.0.1:8080/api/auth/login \
    ...
    -d '{"username":"admin","password":"admin123"}' --max-time 5 > /dev/null
echo "✅ Login admin"
echo ""
