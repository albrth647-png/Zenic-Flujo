...
import type { OrbitalStatus, CodResult } from "@/types/orbital"

// ─── Tipos ──────────────────────────────────────────────────────────────
// El tipo canónico OrbitalStatus vive en @/types/orbital y coincide con la
// respuesta real del backend (/api/orbital/status en src/web/blueprints/orbital.py):
//   { variables, tor: [...], tor_cache: {...}, rcc: [...], cod: [...], tick, ... }
// Antes este componente declaraba su propia interfaz con `tor_results` y `cod`
// como objeto, lo que NO coincidía con el backend → las líneas TOR y el
// indicador COD nunca renderizaban (BUG P0-1).
...
export function OrbitalVisualizer({ size = CANVAS_SIZE }: OrbitalVisualizerProps) {
  ...
  useEffect(() => {
    ...
    // Líneas TOR — el backend devuelve `tor` (array), no `tor_results` (BUG P0-1).
    if (status.tor && status.tor.length > 0) {
      status.tor.forEach((tor) => {
    ...
    // COD en el centro (punto pulsante) — `cod` es un array de CodResult
    // (uno por ciclo RCC). Tomamos el último resultado para el indicador
    // global; si no hay ciclos, asumimos converged (BUG P0-1).
    const codEntries: CodResult[] = status.cod ?? []
    const latestCod = codEntries.length > 0 ? codEntries[codEntries.length - 1] : undefined
    const codConverged = latestCod?.converged ?? true
  ...
  const torCount = status?.tor?.length ?? 0
  const torHitRate = status?.tor_cache?.hit_rate ?? 0
  const codEntries: CodResult[] = status?.cod ?? []
  const latestCod = codEntries.length > 0 ? codEntries[codEntries.length - 1] : undefined
  const codConverged = latestCod?.converged ?? true
  const codIterations = latestCod?.iterations ?? 0
