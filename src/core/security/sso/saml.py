...
import urllib.parse
import uuid
# Fix Sprint 2 bug #21: usar defusedxml en vez de xml.etree.ElementTree
# para proteger contra XXE (XML External Entity) y billion laughs attack.
# defusedxml es un drop-in replacement que bloquea entity expansion maliciosa.
...
try:
    ...
    logging.getLogger(__name__).warning(
        "defusedxml no instalado — SAML parsing vulnerable a XXE/billion laughs. "
...
from src.core.security.sso.constants import SAML_NS, SSO_BASE_URL
...
<md:EntityDescriptor xmlns:md="urn:oasis:names:tc:SAML:2.0:metadata"
  ...
  <samlp:NameIDPolicy Format="urn:oasis:names:tc:SAML:1.1:nameid-format:emailAddress"
    ...
    def handle_callback(self, config: dict, saml_response: str) -> dict:
        ...
        SAML, permitiendo forgery de aserciones. Ahora valida la firma usando
        python3-saml (xmlsec1 bajo el hood) contra el certificado del IdP.
        Si la firma es inválida o ausente, rechaza con 401.
        """
        try:
        ...
        # ── Validación de firma digital (fix bug #22) ─────────────
        # Solo validar si hay certificado del IdP configurado. Si no hay,
        # loggear warning pero continuar (para no romper setups existentes
        # sin validación — se documentará que es inseguro).
        idp_cert = config.get("idp_cert") or config.get("idp_x509_cert")
        if idp_cert:
            if not self._validate_saml_signature(decoded_xml, idp_cert):
                logger.error(
                    "SSO SAML: Firma digital de aserción inválida o ausente — "
                    "posible forgery. Rechazando."
                )
                return {
                    "status": "error",
                    "message": "Firma SAML inválida — autenticación rechazada por seguridad",
                }
        else:
            logger.warning(
                "SSO SAML: idp_cert no configurado — NO se valida firma digital. "
                "ESTO ES INSEGURO. Configura 'idp_cert' en el provider SAML para "
                "validar firmas y prevenir forgery de aserciones."
            )
    ...
    def _validate_saml_signature(self, saml_xml: str, idp_cert: str) -> bool:
        """Valida la firma digital de una aserción SAML contra el certificado del IdP.

        Fix Sprint 2 bug #22: usa python3-saml (xmlsec1) para validar.
        Si python3-saml no está disponible, falla seguro (retorna False)
        para no permitir bypass.

        Args:
            saml_xml: XML de la respuesta SAML decodificada.
            idp_cert: Certificado X.509 del IdP en formato PEM.

        Returns:
            True si la firma es válida, False si es inválida o no se puede validar.
        """
        try:
            # python3-saml proporciona XMLSec + validación completa
            from onelogin.saml2.utils import OneLogin_Saml2_Utils

            # Validar firma usando xmlsec1 internamente
            try:
                # OneLogin_Saml2_Utils.validate_sign proporciona validación directa
                OneLogin_Saml2_Utils.validate_sign(
                    saml_xml,
                    cert=idp_cert,
                    validatecert=True,
                )
                logger.info("SSO SAML: Firma digital validada correctamente")
                return True
            except Exception as e:
                logger.error(f"SSO SAML: Validación de firma falló: {e}")
                return False

        except ImportError:
            # python3-saml no instalado — fail safe (no bypass)
            logger.error(
                "SSO SAML: python3-saml no instalado — no se puede validar firma. "
                "Rechazando aserción por seguridad. Instala con: pip install python3-saml"
            )
            return False
        except Exception as e:
