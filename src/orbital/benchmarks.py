...
class BenchORBITAL:
    ...
    def bench_tor_matrix(self, sizes: list[int] | None = None):
    ...
    def bench_cod_convergence(self, sizes: list[int] | None = None):
    ...
    def bench_cod_amplitudes(self, amplitudes: list[float] | None = None):
        ...
        for amp in amplitudes:
            ...
            print(
                f"  Amp={amp:6.0f}  iters={r.iterations:4d}  "
                f"converged={r.converged}  delta={r.convergence_delta:.8f}  "
                f"{elapsed:.2f}ms"
            )
