...
class CodeSandbox:
    ...
    def execute_python(
        ...
        try:
            ...
            logger.warning(
                "Sandbox: signal.SIGALRM no disponible - ejecución sin timeout",
            )
        ...
        try:
            ...
            try:
                ...
                logger.warning(
                    "Sandbox: no se pudo restaurar signal handler original",
                )
    ...
    def _validate_source(self, code: str) -> str | None:
        ...
        # 1. BLOCKED_BUILTINS (quita eval/exec/open/compile de builtins)
        # 2. SAFE_MODULES + __import__ restringido (solo módulos seguros)
        # 3. AST validation de imports (detecta módulos no permitidos)
        # 4. Contexto aislado sin acceso a __builtins__ original
        # 5. Timeout via signal.SIGALRM
        # No se necesita validación string-based porque el AST walk y
        # las builtins restringidas cubren todos los casos de forma robusta.
