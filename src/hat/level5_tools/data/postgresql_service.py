...
class PostgreSQLService:
    ...
    def update(
        ...
        # Fix Sprint 2 bug #19: validar `where` contra patrón estricto.
        # Antes el `where` era caller-controlled sin validación, lo que permitía
        # SQL injection si un caller malicioso pasaba string arbitrario.
        # Patrón permitido: combinación de "<columna> <op> %s" unidos por AND/OR.
        # Columnas: [a-zA-Z0-9_]+; operadores: =, !=, <>, <, >, <=, >=, LIKE, IN
        # Placeholders: %s (uno o más, entre paréntesis para IN)
        if where:
            where_pattern = r'^\s*[a-zA-Z0-9_]+\s+(?:=|!=|<>|<=|>=|<|>|LIKE|ILIKE|IN\s*\(\s*%s\s*(?:,\s*%s\s*)*\))\s*(?:::[\w]+\s*)?(?:\s+(?:AND|OR)\s+[a-zA-Z0-9_]+\s+(?:=|!=|<>|<=|>=|<|>|LIKE|ILIKE|IN\s*\(\s*%s\s*(?:,\s*%s\s*)*\))\s*(?:::[\w]+\s*)?)*\s*$'
            if not re.match(where_pattern, where, re.IGNORECASE):
                logger.error(
                    f"PostgreSQL update: WHERE clause rechazada por validación: {where!r}"
                )
                return self._error(
                    "WHERE clause inválida. Patrón permitido: "
                    "'columna op %s [AND|OR columna op %s]*'. "
                    "Ejemplo válido: 'id = %s AND status = %s'"
                )
        ...
        sql = f"UPDATE {table} SET {', '.join(set_clauses)} WHERE {where}"  # nosec B608 — table, columns y where validados con regex arriba
