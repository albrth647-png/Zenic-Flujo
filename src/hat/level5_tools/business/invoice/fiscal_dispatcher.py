...
# ── TypedDicts (tipado concreto) ─────────────────────────────────────


class FiscalResult(TypedDict):
    """Resultado estandarizado de una operación fiscal LATAM."""
    success: bool
    country: str
    action: str
...
class AuditLogEntry(TypedDict):
    """Entrada del audit log del FiscalDispatcher."""
    country: str
    action: str
    success: bool
    tracking_id: str
...
class FiscalDispatcher:
    ...
    def __init__(self, event_bus: EventBus | None = None) -> None:
        ...
        self._dispatch_log: list[AuditLogEntry] = []  # audit trail en-memory
    ...
    def dispatch(
        ...
        Returns:
            FiscalResult estandarizado (TypedDict):
            {
                "success": bool,
                "country": str,
                "action": str,
...
def get_dispatcher() -> FiscalDispatcher:
    ...
    Usa ``functools.lru_cache(maxsize=1)``. Para resetear en tests,
    llamar a ``get_dispatcher.cache_clear()``.
    """
    return FiscalDispatcher()
