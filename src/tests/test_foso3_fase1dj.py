...
class TestFrontendMiNegocio:
    ...
    def test_minegocio_page_has_dashboard_layout(self):
        ...
        assert "Mi Negocio" in source
        assert "Leads" in source or "leads" in source.lower()
        assert "Facturas" in source or "invoice" in source.lower()
        assert "Stock" in source or "inventory" in source.lower()
