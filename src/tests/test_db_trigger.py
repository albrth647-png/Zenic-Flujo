...
class TestDatabaseTrigger:
    ...
    def test_trigger_fires_on_lead_insert(self, db_manager):
        ...
        events = db_manager.fetchall(
            "SELECT * FROM event_queue WHERE event_type = 'crm.lead.created'"
        )
        assert len(events) >= 1
    ...
    def test_trigger_fires_on_lead_update(self, db_manager):
        ...
        events = db_manager.fetchall(
            "SELECT * FROM event_queue WHERE event_type = 'crm.lead.updated'"
        )
        assert len(events) >= 1
    ...
    def test_trigger_fires_on_product_insert(self, db_manager):
        ...
        db_manager.commit()
        events = db_manager.fetchall(
            "SELECT * FROM event_queue WHERE event_type = 'inventory.product.created'"
        )
        assert len(events) >= 1
    ...
    def test_trigger_fires_on_invoice_insert(self, db_manager):
        ...
        db_manager.commit()
        events = db_manager.fetchall(
            "SELECT * FROM event_queue WHERE event_type = 'invoice.created'"
        )
        assert len(events) >= 1
    ...
    def test_trigger_fires_on_lead_delete(self, db_manager):
        ...
        db_manager.commit()
        events = db_manager.fetchall(
            "SELECT * FROM event_queue WHERE event_type = 'crm.lead.deleted'"
        )
        assert len(events) >= 1
