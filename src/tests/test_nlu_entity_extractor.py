...
class TestEntityExtractor:
    ...
    def test_extract_number(self):
        ...
        num = [e for e in entities if e.type == "number"][0]
    ...
    def test_trigger_type_event(self):
        from src.nlu.entities.extractor import EntityExtractor
        extractor = EntityExtractor()
        ttype, config = extractor.extract_trigger_type("Cuando un nuevo cliente")
        assert ttype == "event"

    def test_trigger_type_schedule(self):
        from src.nlu.entities.extractor import EntityExtractor
        extractor = EntityExtractor()
        ttype, config = extractor.extract_trigger_type("Cada dia a las 9")
        assert ttype == "schedule"

    def test_trigger_type_webhook(self):
        from src.nlu.entities.extractor import EntityExtractor
        extractor = EntityExtractor()
        ttype, config = extractor.extract_trigger_type("Webhook HTTP")
        assert ttype == "webhook"

    def test_trigger_type_manual_default(self):
        from src.nlu.entities.extractor import EntityExtractor
        extractor = EntityExtractor()
        ttype, config = extractor.extract_trigger_type("Generar backup")
        assert ttype == "manual"
