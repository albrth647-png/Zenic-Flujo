...
class TestWorkflowAIGenerator:
    ...
    def test_parse_workflow_with_extra_text(self):
        ...
        raw = (
            "Aquí está el workflow:\n"
            '{"name": "wf1", "trigger_type": "manual", '
            '"steps": [{"id": 1, "tool": "crm", '
    ...
    def test_parse_missing_name(self):
        ...
        raw = json.dumps(
            {"trigger_type": "manual", "steps": [{"id": 1, "tool": "crm", "action": "create_lead", "params": {}}]}
        )
        parsed = WorkflowAIGenerator._parse_workflow(raw)
    ...
    def test_validate_invalid_tool(self):
        ...
        workflow = {
            "name": "test_wf",
            "trigger_type": "manual",
            "steps": [{"id": 1, "tool": "tool_inexistente", "action": "do_something", "params": {}}],
        }
        errors = WorkflowAIGenerator._validate_workflow(workflow)
        assert len(errors) > 0
    ...
    def test_validate_invalid_trigger_type(self):
        ...
        workflow = {"name": "test_wf", "trigger_type": "invalid_type", "steps": []}
        errors = WorkflowAIGenerator._validate_workflow(workflow)
    ...
    def test_validate_empty_steps(self):
        ...
        workflow = {"name": "test_wf", "trigger_type": "manual", "steps": []}
        errors = WorkflowAIGenerator._validate_workflow(workflow)
    ...
    def test_validate_missing_action(self):
        ...
        workflow = {
            "name": "test_wf",
            "trigger_type": "manual",
            "steps": [{"id": 1, "tool": "crm", "action": "", "params": {}}],
        }
        errors = WorkflowAIGenerator._validate_workflow(workflow)
    ...
    def test_validate_all_known_tools(self):
        from src.nlu.ai_generator import KNOWN_TOOLS, WorkflowAIGenerator

        for tool in KNOWN_TOOLS:
            workflow = {
                "name": f"test_{tool}",
                "trigger_type": "manual",
                "steps": [{"id": 1, "tool": tool, "action": "test_action", "params": {}}],
            }
            errors = WorkflowAIGenerator._validate_workflow(workflow)
...
class TestAIGeneratorLLM:
    ...
    def test_generate_ollama_invalid_json(self):
        ...
        with patch.dict(
            ...
            mock_response = MagicMock()
            mock_response.status_code = 200
            mock_response.json.return_value = {"message": {"content": "No puedo generar eso"}}
            mock_response.raise_for_status = MagicMock()
                        ...
                        "text": json.dumps(
                            {
                                ...
                                "steps": [{"id": 1, "tool": "invoice", "action": "get_overdue_invoices", "params": {}}],
                            }
                        )
                    }
                ]
                    ...
                    "content": json.dumps(
                        {
                            "name": "Bad WF",
                            "trigger_type": "manual",
                            "steps": [{"id": 1, "tool": "nonexistent_tool", "action": "do_stuff", "params": {}}],
                        }
                    )
                }
            }
