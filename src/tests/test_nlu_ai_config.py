...
class TestProviderConfig:
    ...
    def test_is_configured_ollama_with_url(self):
        ...
        config = ProviderConfig(provider=AIProvider.OLLAMA, enabled=True, base_url="http://localhost:11434")
    ...
    def test_is_configured_openai_full(self):
        ...
        config = ProviderConfig(provider=AIProvider.OPENAI, enabled=True, api_key="sk-test", model="gpt-4o-mini")
        assert config.is_configured is True

    def test_is_configured_anthropic_full(self):
        from src.nlu.ai_config import AIProvider, ProviderConfig

        config = ProviderConfig(provider=AIProvider.ANTHROPIC, enabled=True, api_key="sk-ant-test", model="claude-3")
        assert config.is_configured is True
