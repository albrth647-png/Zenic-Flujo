"""
VERIFY F0-D6 — 10 verificaciones del protocolo Code-Forge Agent v2.0
...
HAT_FILES = [
    REPO_ROOT / "src/hat/agents/cards.py",
    REPO_ROOT / "src/hat/agents/card_publisher.py",
...
def test_v01_tests_pass():
    """V01: Todos los tests de F0-D6 pasan.
    ...
    test_path = REPO_ROOT / "src/tests/hat/test_cards_publisher.py"
    spec = importlib.util.spec_from_file_location("test_cards_publisher", test_path)
...
def test_v02_coverage_above_90():
    ...
    test_src = (REPO_ROOT / "src/tests/hat/test_cards_publisher.py").read_text()
            ...
            if isinstance(node, ast.ClassDef):
                for method in node.body:
                    if isinstance(method, ast.FunctionDef) and not method.name.startswith("_"):
                        ...
                        # get_card es implementado en subclases — check loose
