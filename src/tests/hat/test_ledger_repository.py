...
def session():
    ...
    return {
        "user_id": f"user_{datetime.now(timezone.utc).strftime('%H%M%S%f')}",
        "session_id": f"sess_{datetime.now(timezone.utc).strftime('%H%M%S%f')}",
    }
...
class TestSessionsCRUD:
    def test_start_session_creates_new(self, repo):
        sid = f"sess_{datetime.now(timezone.utc).strftime('%H%M%S%f')}"
