...
class ErrorHandler:
    ...
    def handle(self, step: dict, error: Exception, context: dict, step_executor) -> ErrorHandlerResult:
        ...
        continue_on_error = step.get("continue_on_error", False) or context.get("workflow", {}).get(
        ...
        is_timeout = "timeout" in str(error).lower() or "excedio el timeout" in str(error).lower()
        if is_timeout and not retry_on_timeout:
            logger.info(f"ErrorHandler: Timeout en paso {step_id}, no se reintenta (retry_on_timeout=False)")
            if continue_on_error:
                return self._handle_continue_on_error(step)
            self._send_to_dead_letter(step, error, 0, step_executor, context)
        ...
        logger.warning(
            ...
            f"TOR={orbital_alignment:.4f} theta={orbital_theta:.4f} "
            f"{'RECUPERABLE' if orbital_alignment > 0 else 'DIFICIL'}"
        )

        # 3. Ajustar reintentos segun alineacion orbital
        effective_max_retries = max(max_retries, 1) if orbital_alignment > 0 else max(max_retries // 2, 1)
        ...
        for attempt in range(1, effective_max_retries + 1):
            ...
            try:
                ...
                logger.warning(f"Reintento {attempt} fallo: {retry_error}")
        ...
        if continue_on_error:
            logger.info(f"continue_on_error=True para paso {step_id}: enviando a dead letter y continuando")
            self._send_to_dead_letter(step, error, effective_max_retries, step_executor, context)
            return self._handle_continue_on_error(step)
