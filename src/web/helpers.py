...
def init(event_bus_instance: EventBus | None = None, subscriber: WorkflowSubscriber | None = None) -> None:
    ...
    Raises:
        RuntimeError: si subscriber es None (init() debe llamarse desde main.py
            con un subscriber válido antes de servir requests).
    """
    ...
    elif workflow_subscriber is None:
        logger.warning(
            "helpers.init() llamado sin subscriber — endpoints de workflows "
            "fallarán con 503 hasta que se inicialice correctamente"
        )


def get_workflow_subscriber() -> WorkflowSubscriber:
    """Retorna el WorkflowSubscriber global o lanza RuntimeError si no está init.

    Los endpoints deben usar esto en vez de acceder al global directamente,
    para fallar con 503 claro en vez de AttributeError críptico.
    """
    if workflow_subscriber is None:
        raise RuntimeError(
            "WorkflowSubscriber no inicializado. Llama helpers.init(subscriber=...) "
            "desde main.py antes de servir requests."
        )
    return workflow_subscriber
...
def require_role(role: str):
    ...
    def decorator(f):
        ...
        def decorated(*args, **kwargs):
            ...
            if not role_at_least(user_role, role):
